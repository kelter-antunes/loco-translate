...
<div class="wrap loco-admin loco-init"><?php 
    ...
    <form action="" method="get" class="wp-core-ui" id="loco-msginit">
        ...
        <p>
            <select name="common-locale">
                ...
                foreach( $locales as $code => $name ):
                    //do not show all languages for not superadmin
                    if(is_super_admin() || $code == get_locale()) :
                        ?>
        ...
        <p>
            ...
            <input type="<?php if(is_super_admin()){?>text<?php } else { ?>hidden<?php } ?>" maxlength="6" size="5" pattern="^[a-zA-Z]{2,3}([\-_][a-zA-Z]{2})?$" name="custom-locale" for="f-loco-locale" placeholder="xx_XX" />
